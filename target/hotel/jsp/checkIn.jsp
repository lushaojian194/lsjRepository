...
<script type="text/javascript">	
	function ruTime(){	
      ...
      document.getElementById("checkDay").value = realTime;
}   
     setTimeout('ruTime()',500); 
</script>
<script>
           function checkIn() {
               var sign = true;
               ...
               //日期或天数为空时，sign==false,不能提交
               if(sign){
                   $.ajax({
                       url:"${pageContext.request.contextPath}/find/checkIn?location=${roomDetail.location}&price=${roomDetail.price}"+"&checkDay="+checkDay+"&day="+day,
                       type:"post",
                       data:"null",
...
<%--<form action="${pageContext.request.contextPath}/RoomServlet?method=checkIn&userName=${user.userName}&location=${roomDetail.location}&price=${roomDetail.price}" method="post">	--%>
  ...
  <tbody>
  	<tr>
  		<td>请选择入住日期</td>
  		<td>
            <input type="text" id="checkDay" name="checkDay" class="form-control" placeholder="请选择预定日期" readonly="readonly">      
  		</td>
  	</tr>
    <tr>
      <td>请输入入住天数</td>
      <td><input type="text" id="day" name="day" class="form-control" placeholder="请输入住天数"></td>
    </tr>
    <tr>
      <td colspan="2" style="text-align: center;"><button onclick="checkIn()" class="layui-btn layui-btn-normal"><font size="4">确定</font></button></td>
    </tr>
  </tbody>
  </table>
<%--</form>  --%>
